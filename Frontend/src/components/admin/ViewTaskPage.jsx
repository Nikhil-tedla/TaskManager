...
//     const statusColors = {
//         all: "bg-blue-500",
//         pendingTasks: "bg-yellow-500",
//         inProgressTasks: "bg-orange-500",
...
//         <div className={`text-white p-4 rounded-xl shadow-md ${color}`}>
...
//                     <div
//                         key={task._id}
//                         className="relative bg-white shadow-md rounded-xl p-5 border-l-8 min-h-[200px]"
//                         style={{
//                             borderColor:
...
const ViewTaskPage = () => {
    ...
    const handleEdit = (task) => {
        ...
        setFormData({
            ...
            assignedTo: task.assignedTo.map(u => ({ value: u._id, label: u.name })), // array of IDs
        });
    };
                    ...
                    <div key={task._id} className="relative bg-white shadow-md rounded-xl p-5 border-l-8 min-h-[200px]"
                        ...
                        <div className="absolute top-3 right-3 flex space-x-4">
                            {editingTaskId === task._id ? (
                                <>
                                    ...
                                    <button onClick={() => setEditingTaskId(null)} className="text-gray-500"><X/></button>
                                </>
                            ) : (
                                <>
                                    <button onClick={() => handleEdit(task)} className="text-blue-500"><Pencil size={25} /></button>
                                    <button onClick={() => handleDelete(task._id)} className="text-red-500"><Trash2 size={25} /></button>
                                </>
                            )}
                        </div>
                        ...
                        {editingTaskId === task._id ? (
                            <>  {activeField && (
                                ...
                                <input className="w-2/3 mb-1 border rounded p-1" name="title" value={formData.title} onChange={handleChange} onFocus={() => setActiveField("Title")} />
                                <textarea className="w-2/3 mb-2 border rounded p-1" name="description" value={formData.description} onChange={handleChange} onFocus={() => setActiveField("Description")} />
                                ...
                                <select name="priority" className="w-2/3 mb-1 border rounded p-1" value={formData.priority} onChange={handleChange} onFocus={() => setActiveField("Priority")}>
                                    <option>Low</option>
                                    <option>Medium</option>
                                    <option>High</option>
                                </select>
                                <input type="date" name="dueDate" className="w-2/3 mb-1 border rounded p-1" value={formData.dueDate} onChange={handleChange} onFocus={() => setActiveField("dueDate")} />
                                ...
                                <Select
                                    ...
                                    onFocus={() => setActiveField("Assign Users")}
